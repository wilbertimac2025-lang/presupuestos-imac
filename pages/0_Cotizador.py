...
DESC_LEVANTAMIENTO = """PREPARACIÓN DE SUPERFICIE PARA APLICACIÓN DE SISTEMA DE IMPERMEABILIZACIÓN, INCLUYENDO RETIRO DE IMPERMEABILIZANTE EXISTENTE EN MAL ESTADO, LEVANTAMIENTO Y RECOLECCIÓN DE BASURA, RESIDUOS Y MATERIALES SUELTOS; LIMPIEZA GENERAL DE LA SUPERFICIE, DESAZOLVE Y LIMPIEZA DE COLADERAS Y PUNTOS DE DESAGÜE; RETIRO DE MATERIAL DETERIORADO O QUE PUEDA AFECTAR LA ADHERENCIA DEL NUEVO SISTEMA; ACARREO, CARGA Y DESCENSO DE RESIDUOS GENERADOS DURANTE LOS TRABAJOS, ASÍ COMO LIMPIEZA FINAL DEL ÁREA, DEJANDO LA SUPERFICIE DEBIDAMENTE PREPARADA Y EN CONDICIONES ADECUADAS PARA LA APLICACIÓN DEL NUEVO SISTEMA DE IMPERMEABILIZACIÓN."""

DESC_ACRILICO = """RECUBRIMIENTO ELASTICO IMPERMEABLE CON BASE EN RESINAS ACRILICAS DE LARGA VIDA Y EFICIENCIA. IDEAL PARA APLICARSE SOBRE TECHOS Y CUBIERTAS TANTO EN OBRAS NUEVAS, O EN SUPERFICIES QUE YA HABIAN SIDO IMPERMEABILIZADAS."""

ESPEC_ACRILICO = """1. PREPARACIÓN DE LA SUPERFICIE, BARRIDO Y LIMPIEZA DEL ÁREA PARA RETIRAR POLVO Y FALSAS ADHERENCIAS.
2. SELLADO DE GRIETAS Y FISURAS (HASTA 3MM) CON CEMENTO PLÁSTICO.
3. APLICACIÓN DE UNA CAPA DE PRIMARIO SELLADOR.
4. APLICACIÓN DE PRIMERA CAPA DE IMPERMEABILIZANTE ACRÍLICO.
5. APLICACIÓN DE SEGUNDA CAPA DE IMPERMEABILIZANTE EN SENTIDO CRUZADO."""
...
ESPEC_PREFAB = """1. PREPARACIÓN DE LA SUPERFICIE, BARRIDO Y LIMPIEZA DEL ÁREA.
2. TRATAMIENTO DE PUNTOS CRÍTICOS (BAJADAS PLUVIALES, CHAFLANES Y BASES).
3. APLICACIÓN DE HIDROFLEX COMO PRIMARIO SELLADOR PARA ADHERENCIA.
4. TERMOFUSIÓN DEL MANTO PREFABRICADO MEDIANTE SOPLETE DE GAS BUTANO.
5. REMATE Y SELLADO DE TRASLAPES A FUEGO DIRECTO PARA HERMETICIDAD TOTAL."""
...
CATALOGO_SISTEMAS = {
    ...
    "KRIPTOFLEX 3 AÑOS CON MALLA": {"precio": 189.00, "garantia": "3 AÑOS CONTRA DEFECTOS DE FABRICACIÓN", "desc": DESC_ACRILICO, "espec": ESPEC_ACRILICO, "ficha": "ficha_kriptoflex3000.pdf", "color": "BLANCO / ROJO"},
    "KRIPTOFLEX 3 AÑOS FIBRATADO": {"precio": 171.00, "garantia": "3 AÑOS CONTRA DEFECTOS DE FABRICACIÓN", "desc": DESC_ACRILICO, "espec": ESPEC_ACRILICO, "ficha": "ficha_kriptoflex3000.pdf", "color": "BLANCO / ROJO"},
    "KRIPTOFLEX 5 AÑOS FIBRATADO": {"precio": 209.00, "garantia": "12 MESES CONTRA DEFECTOS DE FABRICACIÓN", "desc": DESC_ACRILICO, "espec": ESPEC_ACRILICO, "ficha": "ficha_kriptoflex5000.pdf", "color": "BLANCO / ROJO"},
    "KRIPTOFLEX 5 AÑOS CON MALLA": {"precio": 219.00, "garantia": "12 MESES CONTRA DEFECTOS DE FABRICACIÓN", "desc": DESC_ACRILICO, "espec": ESPEC_ACRILICO, "ficha": "ficha_kriptoflex5000.pdf", "color": "BLANCO / ROJO"},
    ...
    "JUNTA LINEAL 15 A 50 CM KRIPTOFLEX": {"precio": 125.00, "garantia": "NO APLICA", "desc": DESC_ACRILICO, "espec": ESPEC_ACRILICO, "ficha": "NO APLICA", "color": "BLANCO / ROJO"},
...
def enviar_respaldo_correo(pdf_bytes, nombre_archivo, cliente, asesor, folio, tipo_obra, proyecto, ubicacion):
    try:
        ...
        if tipo_obra == "LOCAL": correo_destino = "comercial@grupo-imac.com, ya@grupo-imac.com, pue@grupo-imac.com, pue2@grupo-imac.com"
...
if st.button("GENERAR PRESUPUESTO OFICIAL", type="primary"):
    ...
    if not c_valido or not a_valido or not u_valido or not p_valido: st.error("⚠️ El Cliente, Asesor, Nombre del Proyecto y Ubicación son obligatorios.")
        ...
        with st.spinner("Calculando Rendimientos, Aplicando Descuentos y Ensamblando PDF..."):
            ...
            if compania:
                pdf.set_x(114); pdf.set_font('Arial', 'B', 9); pdf.set_text_color(0, 150, 255); pdf.cell(80, 5, f"{compania.upper()}", ln=True)
            pdf.set_x(114); pdf.set_font('Arial', '', 8); pdf.set_text_color(100, 100, 100)
            cliente_info = ""
            ...
            pdf.multi_cell(0, 4, txt="- Se deberá hacer un levantamiento físico para poder determinar los alcances del trabajo solicitado\n- No incluye trabajos de albañilería no cotizados\n- Trabajos no cotizados."); pdf.ln(3)
